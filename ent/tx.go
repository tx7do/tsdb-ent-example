...
func (tx *Tx) init() {
	tx.AttributeKV = NewAttributeKVClient(tx.config)
	tx.Sensor = NewSensorClient(tx.config)
	tx.SensorData = NewSensorDataClient(tx.config)
	tx.TsKv = NewTsKvClient(tx.config)
	tx.TsKvDictionary = NewTsKvDictionaryClient(tx.config)
	tx.TsKvLatest = NewTsKvLatestClient(tx.config)
}
...
// applies a query, for example: AttributeKV.QueryXXX(), the query will be executed
